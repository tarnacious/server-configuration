...
class MuninSupervisordProcessStatsPlugin(MuninPlugin):
    ...
    def __init__(self, argv=(), env=None, debug=False):
        ...
        graphs = [('supervisord_{{item.name}}_memory_usage', 'Memory usage',
                   'Memory usage', 'Memory usage (MiB)', 'LINE2', 'GAUGE', None),
                  ('supervisord_{{item.name}}_cpu_percent', 'CPU utilization as a percentage',
                   'CPU utilization as a percentage', 'CPU percentage', 'LINE2', 'GAUGE', None),
                  ('supervisord_{{item.name}}_num_fds', 'File descriptors used',
                   'File descriptors used', None, 'LINE2', 'GAUGE', '--lower-limit 0'),
                  ('supervisord_{{item.name}}_num_threads', 'Threads currently used',
                   'Threads currently used', None, 'LINE2', 'GAUGE', '--lower-limit 0'),
                  ('supervisord_{{item.name}}_num_connections', 'Socket connections opened',
                   'Socket connections opened', None, 'LINE2', 'GAUGE', '--lower-limit 0'),
                  ('supervisord_{{item.name}}_uptime', 'Process Uptime',
                   'Process Uptime', None, 'LINE2', 'GAUGE', '--lower-limit 0')

                  ]

        self._category = 'supervisord_{{item.name}}'
        ...
        for (graph_name, graph_title, graph_info, graph_vlabel, graph_draw, graph_type, graph_args) in graphs:
            if self.graphEnabled(graph_name):
                graph = MuninGraph('Supervisor-{{item.name}} - {0}'.format(graph_title),
                                   self._category, info=graph_info, vlabel=graph_vlabel, args=graph_args)

                for entry in self.entries:

                    if not entry["name"].startswith("{{item.name}}"):
                        continue
    ...
    def retrieveVals(self):
        ...
        for entry in self.entries:
            ...
            try:
                ...
                # memory usage
                if self.graphEnabled('supervisord_{{item.name}}_memory_usage')\
                        and data['memory_percent'] is not None:
                    memory_usage = (psutil.virtual_memory().total / 1024 / 1024)\
                        * data['memory_percent'] / 100
                    self._stats['supervisord_{{item.name}}_memory_usage'][entry['name']] = memory_usage

                # cpu percent
                if self.graphEnabled('supervisord_{{item.name}}_cpu_percent'):
                    self._stats['supervisord_{{item.name}}_cpu_percent'][entry['name']] = data['cpu_percent']

                # num threads
                if self.graphEnabled('supervisord_{{item.name}}_num_threads'):
                    self._stats['supervisord_{{item.name}}_num_threads'][entry['name']] = data['num_threads']

                # num fds
                if self.graphEnabled('supervisord_{{item.name}}_num_fds'):
                    self._stats['supervisord_{{item.name}}_num_fds'][entry['name']] = data['num_fds']

                # num connections
                try:
                    if self.graphEnabled('supervisord_{{item.name}}_num_connections'):
                        num_connections = len(p.connections())
                        self._stats['supervisord_{{item.name}}_num_connections'][entry['name']] = num_connections
                ...
                try:
                    if self.graphEnabled('supervisord_{{item.name}}_num_connections'):
                        num_connections = len(p.connections())
                        self._stats['supervisord_{{item.name}}_uptime'][entry['name']] = entry['now'] - entry['start']
                except psutil.AccessDenied:
                    pass
